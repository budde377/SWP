...
let val cl = validate load_xml("cardlist.xml") with Cardlist

fun convertList(val cl as Cardlist) : Html_html =
  match cl with
    b:cardlist[b:title[val t], val cards] 
      -> html[head[title[]], body[t, convertCards(cards)]]

fun convertCards(val cs as Card* ) : Html_div* =
  match cs with
    b:card[b:name[val n],b:title[val t],b:email[val e],Any],val rest
      -> div[h2[n],t,br[],"Email: " ^ e],convertCards(rest)
  | () -> ()
